...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d', viz=None):
    ...
    def render_shape(self, vertices, transformed_vertices, albedo=None, images=None, detail_normal_images=None, lights=None):
        '''
        ...
        if albedo is not None:
            face_colors = util.face_vertices(albedo, self.faces.expand(batch_size, -1, -1))
        else:
            face_colors = self.face_colors.expand(batch_size, -1, -1, -1)
    ...
    def render_error(self, transformed_vertices, error):
        '''
        -- rendering depth
        '''
        batch_size = transformed_vertices.shape[0]
        # Attributes
        attributes = util.face_vertices(error, self.faces.expand(batch_size, -1, -1))
        # rasterize
        transformed_vertices[:,:,2] = transformed_vertices[:,:,2] + 10
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)

        ####
        error_images = rendering[:, :3, :, :]
        return error_images
    ...
    def image2verts(self, trans_verts, image):
        batch_size = image.shape[0]
        image_size = image.shape[-1]

        trans_verts = trans_verts[:,:,:2] * image_size/2 + image_size/2
        top_left = trans_verts.long().clamp(0, image_size-1)
        top_right = top_left.clone(); top_right[:,:,0] += 1; top_right = top_right.clamp(0, image_size-1)
        bottom_left = top_left.clone(); bottom_left[:,:,1] += 1; bottom_left = bottom_left.clamp(0, image_size-1)
        bottom_right = bottom_left.clone(); bottom_right[:,:,0] += 1; bottom_right = bottom_right.clamp(0, image_size-1)

        top_left_val = get_pixel_value(image, top_left[:,:,1], top_left[:,:,0]) * ((bottom_right[:,:,0] - trans_verts[:,:,0]) * (bottom_right[:,:,1] - trans_verts[:,:,1])).unsqueeze(-1)
        top_right_val = get_pixel_value(image, top_right[:,:,1], top_right[:,:,0]) * ((trans_verts[:,:,0] - top_left[:,:,0]) * (bottom_right[:,:,1] - trans_verts[:,:,1])).unsqueeze(-1)
        bottom_left_val = get_pixel_value(image, bottom_left[:,:,1], bottom_left[:,:,0]) * ((bottom_right[:,:,0] - trans_verts[:,:,0]) * (trans_verts[:,:,1] - top_left[:,:,1])).unsqueeze(-1)
        bottom_right_val = get_pixel_value(image, bottom_right[:,:,1], bottom_right[:,:,0]) * ((trans_verts[:,:,0] - top_left[:,:,0]) * (trans_verts[:,:,1] - top_left[:,:,1])).unsqueeze(-1)

        verts_vals = top_left_val + top_right_val + bottom_left_val + bottom_right_val
        return verts_vals
