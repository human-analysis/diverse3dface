...
class FLAME(nn.Module):
    ...
    def get_vis_lmks(self, occ_idx):
        lmk_faces = self.faces_tensor[self.full_lmk_faces_idx[0]]
        vis_lmks = np.ones(lmk_faces.shape[0], dtype=int)
        for i, indices in enumerate(lmk_faces):
            for idx in indices:
                if idx.item() in occ_idx:
                    vis_lmks[i] = 0
                    continue
        return vis_lmks


    def get_3d_landmarks(self, vertices, vis_idx=None):
        landmarks3d = vertices2landmarks(vertices, self.faces_tensor,
                                       self.full_lmk_faces_idx.repeat(vertices.shape[0], 1),
                                       self.full_lmk_bary_coords.repeat(vertices.shape[0], 1, 1))
        if vis_idx is not None:
            occ_idx = (1-vis_idx).astype('bool')
            landmarks3d[:,occ_idx] = 0
        return landmarks3d

    def get_landmarks(self, vertices, pose, vis_idx=None):
        batch_size= vertices.shape[0]
        lmk_faces_idx = self.lmk_faces_idx.unsqueeze(dim=0).expand(batch_size, -1)
        lmk_bary_coords = self.lmk_bary_coords.unsqueeze(dim=0).expand(batch_size, -1, -1)

        dyn_lmk_faces_idx, dyn_lmk_bary_coords = self._find_dynamic_lmk_idx_and_bcoords(
            pose, self.dynamic_lmk_faces_idx,
    ...
    def get_landmarks_vis(self, vertices, pose, vis_idx):
        batch_size= vertices.shape[0]
        lmk_faces_idx = self.lmk_faces_idx.unsqueeze(dim=0).expand(batch_size, -1)
        lmk_bary_coords = self.lmk_bary_coords.unsqueeze(dim=0).expand(batch_size, -1, -1)
        ...
        landmarks3d = vertices2landmarks(vertices, self.faces_tensor,
                                       self.full_lmk_faces_idx.repeat(vertices.shape[0], 1),
                                       self.full_lmk_bary_coords.repeat(vertices.shape[0], 1, 1))
        landmarks_vis = vertices2landmarks(vis_idx, self.faces_tensor, lmk_faces_idx, lmk_bary_coords)

        return landmarks2d, landmarks3d, landmarks_vis

    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None, full_shape=None, component_pca_config=None, neck_pose_params=None):
        """
            Input:
        ...
        # for component wise model
        betas = torch.cat([shape_params, expression_params], dim=1)
        if component_pca_config is not None:
            n_baseshape = component_pca_config['n_baseshape']
            n_baseexp = component_pca_config['n_baseexp']
            shapedirs = torch.cat((self.shapedirs[:,:,:n_baseshape], self.shapedirs[:,:,self.config.shape_params:self.config.shape_params+n_baseexp]), dim=2)
        else:
            shapedirs = self.shapedirs

        full_pose = torch.cat([pose_params[:, :3], neck_pose_params.expand(batch_size, -1), pose_params[:, 3:], eye_pose_params], dim=1)
        template_vertices = self.v_template.unsqueeze(0).expand(batch_size, -1, -1)

        # import ipdb; ipdb.set_trace()
        if shape_params is not None and full_shape is not None:
            betas = torch.cat([shape_params, expression_params], dim=1)
            vert_posed, _, vertices = lbs(betas, full_pose, template_vertices, self.shapedirs, self.posedirs, self.J_regressor, self.parents, self.lbs_weights, dtype=self.dtype, component_params=component_params, component_pca=component_pca)
            gt_vertices, _ = lbs_fullshape(full_shape, full_pose, template_vertices, self.posedirs, self.J_regressor, self.parents, self.lbs_weights, dtype=self.dtype)
        elif full_shape is not None:
            gt_vertices, _ = lbs_fullshape(full_shape, full_pose, template_vertices, self.posedirs, self.J_regressor, self.parents, self.lbs_weights, dtype=self.dtype)
        else:
            betas = torch.cat([shape_params, expression_params], dim=1)
            vert_posed, _, vertices = lbs(betas, full_pose, template_vertices,
                              shapedirs, self.posedirs,
                              self.J_regressor, self.parents,
                              self.lbs_weights, dtype=self.dtype,
                              component_pca_config=component_pca_config)
        ...
        landmarks2d = vertices2landmarks(vert_posed, self.faces_tensor,
                                       lmk_faces_idx,
                                       lmk_bary_coords)
        bz = vertices.shape[0]
        landmarks3d = vertices2landmarks(vert_posed, self.faces_tensor,
                                       self.full_lmk_faces_idx.repeat(bz, 1),
                                       self.full_lmk_bary_coords.repeat(bz, 1, 1))

        if shape_params is not None and full_shape is not None:
            return vert_posed, landmarks2d, landmarks3d, vertices, gt_vertices.detach()
        else:
            return vert_posed, landmarks2d, landmarks3d, vertices
