...
def plot_kpts(image, kpts, valid=None, color = 'r'):
    ''' Draw 68 key points
    Args:
    ...
    for i in range(kpts.shape[0]):
        ...
        # image = cv2.putText(image, '{}'.format(i+1), (st[0], st[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.2, color=(0,0,255), thickness=1)
        if i in end_list:
            continue
        ed = kpts[i + 1, :2]
...
def plot_verts(image, kpts, valid=None, color = 'r'):
    ''' Draw 68 key points
    Args:
        image: the input image
        kpt: (68, 3).
...
def tensor_vis_landmarks(images, landmarks, gt_landmarks=None, valid=None, color='g', isScale=True, draw_line=True):
    # visualize landmarks
    if valid is None:
        valid = [None] * images.shape[0]
    vis_landmarks = []
    ...
    for i in range(images.shape[0]):
        ...
        image = (image * 255)
        if isScale:
            predicted_landmark = (predicted_landmarks[i] * image.shape[0] / 2 + image.shape[0] / 2).astype(int)
        else:
            predicted_landmark = predicted_landmarks[i]

        if draw_line:
            image_landmarks = plot_kpts(image, predicted_landmark, valid[i], color)
            if gt_landmarks is not None:
                image_landmarks = plot_verts(image_landmarks,
                                             (gt_landmarks_np[i] * image.shape[0] / 2 + image.shape[0] / 2).astype(int), 'r')
        else:
            image_landmarks = plot_verts(image, predicted_landmark, valid[i], color)
            if gt_landmarks is not None:
                image_landmarks = plot_verts(image_landmarks,
                                             (gt_landmarks_np[i] * image.shape[0] / 2 + image.shape[0] / 2).astype(int), 'r')
...
# BFM helper functions
def get_camera_matrix(m):
    m = m.view(-1, 4, 2)
...
def get_bfm_landmarks(mshape, m, kpts):
    ...
    vertex2d = torch.matmul(vertex4d, m)[:,:,:2]
    return vertex3d[:,kpts], vertex2d[:,kpts]

def l2_distance(verts1, verts2, valid=None, conf=None):
    diff_norm = torch.sqrt(((verts1 - verts2) ** 2).sum(2))
    if valid is not None and valid[0] is not None:
        diff_norm = diff_norm * valid * valid.shape[-1] / valid[0].sum()
    if conf is not None and conf[0] is not None:
        diff_norm = diff_norm * conf
